...
from app.core.logging import log_user_action, log_security_event, SecurityEvent
...
async def list_users(
    ...
    client_ip = None
    try:
        from fastapi import Request as _Request  # local import to avoid circular if any
...
async def deactivate_user(
    ...
    if user.is_superuser:
        statement = select(User).where(User.is_superuser == True, User.is_active == True)
        result = await session.execute(statement)
