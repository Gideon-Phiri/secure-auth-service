...
async def test_update_profile(client: AsyncClient, verified_user):
    ...
    assert update_response.json()["email_verified"] is False  # Reset on email change
...
async def test_admin_create_user(client: AsyncClient, admin_user):
    ...
    assert (
        create_response.json()["email_verified"] is True
...
async def test_admin_activate_deactivate_user(
    client: AsyncClient, admin_user, verified_user
):
    ...
    activate_response = await client.post(
        f"/users/{user_id}/activate", headers={"Authorization": f"Bearer {admin_token}"}
    )
