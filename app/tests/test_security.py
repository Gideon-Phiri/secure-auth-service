...
async def test_auth_failure_logging():
    ...
    with patch('app.core.logging.logger') as mock_logger:
        log_auth_failure("test@example.com", "192.168.1.1", "Invalid password", "Mozilla/5.0")
...
async def test_login_attempts_are_logged(client: AsyncClient, verified_user: User):
    ...
    with patch('app.api.v1.auth.log_auth_success') as mock_success, \
        ...
        # Failed login
        fail_response = await client.post("/auth/login", json={"email": email, "password": "wrongpassword"})
...
async def test_admin_actions_are_logged(client: AsyncClient, admin_user: User):
    ...
    # Login as admin
    login_response = await client.post("/auth/login", json={"email": admin_email, "password": password})
    assert login_response.status_code == 200
    admin_token = login_response.json()["access_token"]
    
    # Patch the names actually imported in users router
    with patch('app.api.v1.users.log_user_action') as mock_log_action, \
         patch('app.api.v1.users.log_security_event') as mock_log_event:
        
        # List users (should log action)
        list_response = await client.get("/users/", headers={"Authorization": f"Bearer {admin_token}"})
...
async def test_rate_limit_logging(client: AsyncClient):
    ...
    with patch('app.core.logging.log_rate_limit_exceeded') as mock_log:
        # This test assumes rate limiting is working
        # Make multiple rapid requests to trigger rate limit
        for _ in range(12):  # Exceed the 10/minute limit for login
            try:
                await client.post("/auth/login", json={"email": "test@test.com", "password": "wrong"})
            except Exception:
