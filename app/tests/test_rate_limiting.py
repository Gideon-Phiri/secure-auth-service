...
from app.core.rate_limit import enable_test_rate_limits, disable_test_rate_limits, reset_rate_limit_wrappers, conditional_limit, is_testing
...
async def test_registration_rate_limit(client: AsyncClient):
    ...
    try:
        ...
        success_or_validation = sum(1 for status in responses[:5] if status in [201, 400])
        ...
        assert success_or_validation >= 1, f"Expected some successful requests, got statuses: {responses[:5]}"
        assert rate_limited >= 1, f"Expected some rate limited requests, got statuses: {responses[5:]}"
...
async def test_login_rate_limit(client: AsyncClient):
    ...
    try:
        ...
        assert unauthorized >= 1, f"Expected some unauthorized responses, got: {responses[:10]}"
        assert rate_limited >= 1, f"Expected some rate limited responses, got: {responses[10:]}"
